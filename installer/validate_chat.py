...
def main(argv=None):
 p=argparse.ArgumentParser(); p.add_argument('--agent',default='Billy'); p.add_argument('--config',default=str(Path.home()/'.config/otacon/config.json')); a=p.parse_args(argv)
 ...
 d=Deployment('deployment_local',[Host(svc.placement_resource_id)],services=[svc]); provider=OllamaProvider(s.get('endpoint',''))
 h=provider.health(s.get('model','')); print(f'Agent: {agent["display_name"]}\nProvider: Ollama\nService: {svc.id}\nModel: {s.get("model","")}\nHealth: {h.state}\nPrompt: What is your name?')
