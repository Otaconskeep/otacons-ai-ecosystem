...
def create_agent(name, index=1, role='primary', voice='default', personality='friendly', avatar=None):
 return {'id':f'agent_{index:03d}','display_name':name.strip() or 'Assistant','role':role,'voice':voice,'personality':personality,'avatar':avatar}
def build_config(plan,name,features,branding=None,storage=None,agents=None):
